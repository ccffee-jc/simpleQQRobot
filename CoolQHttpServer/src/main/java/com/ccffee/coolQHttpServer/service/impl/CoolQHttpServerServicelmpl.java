...
public class CoolQHttpServerServicelmpl implements CoolQHttpServerService {
    ...
    private HashMap groupMassage(HashMap param){
        ...
        cqGroupExample.createCriteria().andNumEqualTo(groupId.toString());
        List<CqGroup> cqGroupList = cqGroupMapper.selectByExample(cqGroupExample);
        CqGroup cqGroup = cqGroupList.get(0);
        if (message.equals("/连接")){
            cqGroup.setConnect(1);
            cqGroupMapper.updateByPrimaryKey(cqGroup);

            HashMap map = new HashMap();
            map.put("reply", "连接已经设置");
            return map;
        }
        if (cqGroup.getConnect() != 0){
            CqGroupExample cqGroupExampleElse = new CqGroupExample();
            cqGroupExampleElse.createCriteria().andConnectEqualTo(cqGroup.getConnect());
            List<CqGroup> cqGroupListElse = cqGroupMapper.selectByExample(cqGroupExampleElse);

            for (CqGroup group: cqGroupListElse){
                if (group.getNum().equals(groupId))
                    continue;
                CommonPost.sendGroupMsg(group.getNum(), nickname + ":" + message);
            }
